        ...
        <Col md={{ span: 6, offset: 3 }}>
          {exam.map((quiz, idx) => {
            return (
              <Card key={idx}>
                {quiz.isRetest ? 
                  <Card.Header as="h5">{quiz.date}  <span style={{color:"red"}}>(RE-TEST)</span></Card.Header> 
                  :
                ...
                <Card.Body>
                  ...
                  <Card.Text>Pass or Fail : {quiz.isPass ? <span style={{color:'green'}}>Pass</span> : <span style={{color:'red'}}>Fail</span>}</Card.Text>
                  <Button
                    variant="info"
                    onClick={(event) => {
